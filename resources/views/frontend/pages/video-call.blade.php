        ...
        <div class="col-12">
            @if($loginRequired)
                <div class="row status-info login-required" id="video-call-info">
                    ...
                    <div><h3 style="width: 100%; color: #fff; font-weight: bold">Please login below to join video
                            call</h3></div>
                    <div class="btn-container">
                        <a class="m-menu__link  btn-outline-primary login-btn"
                           href="{{ route('home') }}/?login_modal=true{{ request()->call_type ? '&redirect_url='.url()->current().'?call_type='.request()->call_type : '' }}"
                           style="width: auto !important;">Login</a>
                    </div>
                </div>
            @elseif(!$callExist)
                <div class="emptyState text-center p-t-60 p-b-40" style="min-height: 65vh">
                    <i class="fa fa-video-camera p-b-20"></i>
                    <h4>Video Call Schedule not found</h4>
                </div>
            @else
                ...
                <div class="" style=" margin: 0 72px">
                    ...
                    <div id="control-buttons" style="display: none;">
                        ...
                        @if($callData['is_scheduler'])
                            <button type="button"
                                    class="video-buttons btn-icon btn-danger addDevice bmd-btn-fab mx-3"
                                    id="button-extra-device" data-container="body" data-toggle="m-tooltip"
                                    data-placement="top" data-original-title="Add Device">
...
@section('page_scripts')
    ...
    @if($callExist)
        <script>
            ...
            $('body').on('click', '#button-extra-device', function () {
                $(resultModal).modal('show')

                $.ajax({
                    url: '{{ route('educator.extra-device.email') }}',
                    data: {
                        _token: '{{ csrf_token() }}',
                        class_id: '{{ $callData['schedule_id'] }}'
                    },
                    type: 'GET',
        ...
        <script type="text/javascript">
            ...
            var callType = '{{ request()->call_type ? request()->call_type : 'video_call' }}'
