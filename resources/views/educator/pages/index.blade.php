...
@section('page_styles')
    <style type="text/css">
        ...
        .job-board-link .m-menu__link-badge {
            position: absolute !important;
            right: -10px;
            top: -10px;
        }

        .job-board-link .m-menu__link-badge .m-badge {
            color: #ffffff;
            background: #e74b65;
            ...
            <div class="row col-12" style="margin-left:0px; margin-right:0px">
                ...
                <div class="col-xl-12 col-md-12 col-sm-12 col-xs-12" style="margin: 0 auto;">
                    <div class="m-content page-dashboard initial-dash">
                        ...
                        <div class="row">
                            <div class="col-xl-3 col-lg-4 padding-mobile-none-lr col-md-eq-height">
                                <div class="row">
                                        ...
                                        <div class="profile-side-nav">
                                            <div class="m-portlet">
                                                <div class="m-portlet__body">
                                                    ...
                                                    @include('educator.includes.left-menu', ['page' => 'overview'])
                            ...
                            <div class="col-xl-9 col-lg-8 padding-mobile-none-lr col-eq-height">
                                <!--starts: Earning Table -->
                            @include('educator.includes.stats-table')
                            <!--end: Earning Table -->
                            </div>
...
@section('page_scripts')
    ...
    <script type="text/javascript">
        $('#uploadID').modal('show')
        $('#call-scheduled').modal('show')
    </script>
    {{--@if(session()->get('show_trusted') && !Auth::user()->trusted)
        <script type="text/javascript">
            $('#trusted').modal('show')
            $('#call-scheduled').modal('show')
        </script>
        @php session()->forget('show_trusted') @endphp
    @endif--}}
    @if($draftHtml)
        <script type="text/javascript">
            console.log('draft')
            $(resultModal).find('div.modal-body').html(`{!! $draftHtml !!}`)
    ...
    <script type="text/javascript">
        ...
        var drafts = parseInt('{{ $draftHtml ? 1 : 0 }}')

        $('#trusted').on('hidden.bs.modal', function () {
            if (drafts == 1) {
                $(resultModal).find('div.modal-body').html(`{!! $draftHtml !!}`)
                $(resultModal).modal('show')
            }
        ...
        $('body').on('click', 'button.delete-draft', function () {
            var _this = $(this)
            $.ajax({
                type: 'POST',
                url: $(this).data('url'),
        ...
        $('body').on('click', 'button.pause-set-route-btn', function () {
            pauseRoute = $(this).data('route')
        })

        $('body').on('click', 'button.live-set-route-btn', function () {
